...
**隐私**：只发送元素的结构化描述（标签名、class、id、尺寸、位置、最多 140 字短文本），**不上传网页正文、不截图、不上传 Cookie**。短文本与属性可在设置中关闭。请求直接发往你填写的地址，不经过任何第三方中转。
...
ui/                      popup + 控制台（规则/AI/扫描/待审/统计）+ 下载工具箱（视频/图片/网盘/下载器）
...
npm run selftest    # 规则解析/导入导出/DNR 编译/例外与作用域的沙箱自测
npm test            # 以上三项
