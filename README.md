...
[![Rules](https://img.shields.io/badge/rules-39%20packs%20·%207270-1fa971)](#-规则)
...
**🌍 39 个规则包 · 7270 条规则** — 覆盖中/日/韩/俄/欧/东南亚/印度/拉美/港澳台站点，全部可独立开关
...
内置 **39 个规则包、7270 条规则**，按分组独立开关；选择器一律做词边界处理（`[class^='ad-slot']` 而非 `[class*='ad-slot']`），不会误伤 `download-slot` 这类正常类名。冲突时：自定义规则 > 内置包 > 订阅规则。
...
```bash
npm test             # check + validate + selftest（146 项检查 · 规则校验 · 185 条断言）
...
目录：`background/` 后台引擎（规则编译/AI/扫描/嗅探）· `content/` 内容脚本（净化/检测/瀑布流补全/特效）· `ui/` 三个界面 · `rules/` 39 个规则包 · `i18n/` 中英词典
