...
| 瀑布流补全 | 自动清除信息流/网格里被拦截广告留下的空位（空壳广告块、幽灵占位），后续内容自然回填，页面不留洞；有真实内容的节点（含懒加载中）一律不动 |
...
| 视频嗅探 | 监听页面 m3u8/mpd 请求 + 扫描页面 DOM/脚本/链接/`data-*` 属性，**不点播放也能发现视频流**；弹窗内直接创建下载任务（进度保留），也可进工具箱批量下载；自动取页面标题（去掉站点名），fMP4 流直接存 `.mp4` |
| 视频下载 | 分片级断点续传（IndexedDB 持久化，取消/失败/关页后可继续）、多线程并发（1-12）、自动选最高画质、分片地址失效自动重拉播放列表、合并前完整性校验、保存成功自动清理缓存分片 |
| 图片发现 | 扫描页面图片，按格式/尺寸筛选、批量下载到 `ZeroZen/图片` |
| 网盘资源 | 自动提取网盘链接与提取码（百度/阿里/夸克/蓝奏/115/天翼/123/迅雷等），一键复制或导出 |
| 下载器 | 内置任务管理器：新建直链下载、实时进度/速度、暂停/继续/取消/重试、打开文件/所在文件夹、清除记录；支持多线程 Range 分段加速（服务器支持时）；按状态/大小/速度/字母/状态排序，失败原因中文化并附解决建议 |
| 下载管理 | 弹窗「任务中心」+ 工具箱下载器：类型筛选（视频/音频/图片/压缩包/文档）与状态筛选、五种排序、进度与速度实时展示；文件操作：打开、Finder 定位（可拖入隔空投送）、复制链接、复制路径、失败重试；失败一键生成诊断报告并打开 GitHub 反馈页（报告自动复制到剪贴板） |
| 分目录归档 | 下载按类型自动归档：`ZeroZen/视频`、`ZeroZen/音频`、`ZeroZen/图片`、`ZeroZen/压缩包`、`ZeroZen/文档`、`ZeroZen/下载`，方便分门别类管理 |
| 界面 | 液态玻璃（Liquid Glass）设计语言：深色玻璃面板、backdrop blur、高光描边、胶囊控件；弹窗内置「本页媒体」与分类「任务中心」（视频/音频/图片/压缩包/文档） |
| 魔法特效 | 一键去除广告播放「净化波」扫光 + 净化徽章；下载创建有粒子迸发特效；阅读模式入场播放页面收束动画（复杂 → 干净的全过程） |
...
- 下载器的多线程分段依赖服务器支持 `Range`，单文件上限 2GB；分片断点续传数据缓存在本地 IndexedDB，任务完成后自动清理，也可在「未完成任务」列表手动删除。普通模式由浏览器接管，可关闭页面、支持断点续传。
